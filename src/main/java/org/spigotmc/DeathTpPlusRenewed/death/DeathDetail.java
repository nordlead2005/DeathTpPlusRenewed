		...
		try {
			...
			if (damageEvent instanceof EntityDamageByEntityEvent) {
				...
				if (damager instanceof Player) {
					...
					if (((Player) damager).getInventory().getItemInMainHand().getType().equals(Material.AIR)) {
						causeOfDeath = DeathEventType.PVP_FISTS;
					} else {
						causeOfDeath = DeathEventType.PVP;
					}
					murderWeapon = ((Player) damager).getInventory().getItemInMainHand().getType().toString();
