...
public class TombWorker {
	...
	public void setPluginInstance(DeathTpPlusRenewed pluginInstance) {
	...
	public boolean economyCheck(Player player, String action) {
		...
		if ((tombCost > 0) && !player.hasPermission("deathtpplusrenewed.tomb.free")) {


			if (pluginInstance.isEconomyActive()) {
				if (pluginInstance.getEconomy().getBalance(player) > tombCost) {
					pluginInstance.getEconomy().withdrawPlayer(player, tombCost);
