...
import org.spigotmc.DeathTpPlusRenewed.tomb.persistence.TombLog;
...
public class Tomb {
		...
		if (!signBlocks.isEmpty()) {
			...
			for (Block block : signBlocks) {
				if (isSign(block)) {
					...
					signBlocks.remove(block);
					block.getWorld().dropItem(block.getLocation(), new ItemStack(Material.OAK_SIGN, 1));
	...
	private boolean isSign(Block sign) {
		...
		sign.getChunk().load();
		return ((sign.getType() == Material.ACACIA_WALL_SIGN ||
		sign.getType() == Material.BIRCH_WALL_SIGN ||
		sign.getType() == Material.DARK_OAK_WALL_SIGN ||
		sign.getType() == Material.JUNGLE_WALL_SIGN ||
		sign.getType() == Material.OAK_WALL_SIGN ||
		sign.getType() == Material.SPRUCE_WALL_SIGN ||
		sign.getType() == Material.ACACIA_SIGN ||
		sign.getType() == Material.BIRCH_SIGN ||
		sign.getType() == Material.DARK_OAK_SIGN ||
		sign.getType() == Material.JUNGLE_SIGN ||
		sign.getType() == Material.OAK_SIGN ||
		sign.getType() == Material.SPRUCE_SIGN)
